...
rally --debug --plugin-paths nova_live_migration.py task start task.json --task-args '{"image_name": "$image_name", "flavor_name": "$flavor_name", "block_migration": true, "host_to_evacuate": "$host_to_evacuate", "destination_host": "$destination_host"}'
